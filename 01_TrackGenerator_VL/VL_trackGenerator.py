...
class TrackGenerator:
    ...
    def generate_randomTrack():
        ...
        track_data = [(round(point[0], 2), round(point[1], 2))
    ...
    def not_connected_track_element(max_xy, track):
        ...
        # somwhere on circle around (0,0) with r_max=max_xy
        r = uniform(0.3*max_xy, max_xy)
        # POINT_UT FROM ALPHA
        alpha = uniform(-MAX_ALPHA, MAX_ALPHA)
        newTan = (tangent_in[0] * np.cos(alpha) + tangent_in[1] * np.sin(alpha), -tangent_in[0]
                  * np.sin(alpha) + tangent_in[1] * np.cos(alpha))  # direction towards point out
        point_out = (point_in[0]+newTan[0]*r,
                     point_in[1]+newTan[1]*r)  # move by r
        # TANGENT_OUT FROM BETA
        beta = uniform(-MAX_BETA, MAX_BETA)
        # beta=MAX_BETA
        tangent_out = (newTan[0] * np.cos(beta) + newTan[1] *
                       np.sin(beta), -newTan[0]*np.sin(beta) + newTan[1] * np.cos(beta))
        done = False
        counter = 0
        rand_track = []
    ...
    def add_straight(point_in,
        ...
        # Prepare outputs
        straight_func = TrackGenerator.parametric_straight(
    ...
    def get_cones(xys, params={}):
        ...
        rand_cones = TrackGenerator.add_random_cone(max_x, max_y, min_x, min_y, choices(
            [0, 1, 2, 3], weights=(p*100, (100-p*100)*0.75, (100-p*100)*0.2, (100-p*100)*0.05)))

        to_return.extend(rand_cones)
        return to_return

    def add_random_cone(max_x,  max_y, min_x, min_y, n):
        r_cones = [(uniform(min_x*1.2, max_x*1.2), uniform(min_y *
                    1.2, max_y*1.2), choice([2, 1]), 2) for _ in range(n[0])]
        r_cones = [(round(point[0], 2), round(point[1], 2),
                    point[2], point[3]) for point in r_cones]
        return r_cones
